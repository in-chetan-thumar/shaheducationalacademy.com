                ...
                <div class="btn-group" role="group">
                    ...
                    <div class="dropdown-menu">
                            <a class="dropdown-item" onclick="showEditModel(event)"
                               href="{{route('assignments.edit',$row->id)}}">Edit</a>

                        {{-- @if($row->is_active == 'Y')
                            <a class="dropdown-item" href="{{ route('assignments.status',$row->id) }}">Inactive</a>
                        @else
                            <a class="dropdown-item" href="{{ route('assignments.status',$row->id) }}">Active</a>
                        @endif --}}
                                <a class="dropdown-item"
                                   onclick="if(confirm('Are you sure you want to delete.')) document.getElementById('delete-{{ $row->id }}').submit()">
                                   Delete</a>
                                <form id="delete-{{ $row->id }}"
                                      action="{{ route('assignments.destroy', $row->id) }}"
                                      method="POST">
                                    @method('DELETE')
                                    @csrf
                                </form>
